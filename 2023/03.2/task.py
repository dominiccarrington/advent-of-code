        ...
        for charNo in range(0, len(line)):
            ...
            for coords in searchOptions:
                ...
                if maybeDigit not in DIGITS:
                    lineCache[coords[0]] = False
                    continue
                elif lineCache.get(coords[0], False):
                    continue
                lineCache[coords[0]] = True

                numbersCache.append(findNumber(lines[coords[0]], coords[1]))

            if len(numbersCache) == 2:
                sigma += numbersCache[0] * numbersCache[1]
